...
SELECT cl.Name
      ...
      ,(array_agg(ARRAY[COALESCE(sname2.name, 'Sector ' || sw.sector),sw.Similarity::text] ORDER BY sw.Similarity DESC))[1:5] AS closestSectors
      ,(array_agg(ARRAY[COALESCE(iname2.name, 'Industry ' || iw.industry),iw.Similarity::text] ORDER BY iw.Similarity DESC))[1:5] AS closestIndustries
