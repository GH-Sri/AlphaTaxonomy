...
# DO NOT RUN THIS SCRIPT - USE the 7 split files 
...
source("Wikipedia_Read/Wiki Functions/wiki_clean.R")
...
write.csv(companies,file = paste0("traded_companies_",".csv"))
...
write.csv(accepted_sites,file = paste0("succeeded_getting_website_",".csv"))
write.csv(exceptions_sites, file = paste0("failed_to_get_website_",".csv"))
...
write.csv(scraped_sites, file = paste0("metadata_on_accepted_sites",".csv"))
...
write.csv(matched, file = paste0("wiki_company_names_crosswalk_",".csv"))
write.csv(wiki_complete, file = paste0("wiki_with_companylist_name_data_",".csv"))
...
write.csv(final_data,file = paste0("public_data_for_doc2vec",".csv"))